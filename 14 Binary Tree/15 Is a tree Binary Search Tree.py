...
def is_tree_BST(root):
    if root==None:
        bp=BSTpair()
        return bp
    lp=is_tree_BST(root.left)
    rp=is_tree_BST(root.right)
    my_pair=BSTpair()
    my_pair.isBSt=lp.isBSt and rp.isBSt and (root.data>=lp.max and root.data<=rp.min)
    my_pair.min=min(root.data,lp.min,rp.min)
    my_pair.max=max(root.data,lp.max,rp.max)
    return my_pair
